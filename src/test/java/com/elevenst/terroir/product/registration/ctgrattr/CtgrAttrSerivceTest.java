package com.elevenst.terroir.product.registration.ctgrattr;


import com.elevenst.terroir.product.registration.catalog.controller.CatalogAttrController;
...
import com.elevenst.terroir.product.registration.ctgrattr.controller.CtgrAttrController;
...
@SpringBootTest
public class CtgrAttrSerivceTest {

    @Autowired
    CtgrAttrMapper ctgrAttrMapper;

    @Autowired
    CtgrAttrServiceImpl ctgrAttrService;

    @Autowired
    CtgrAttrController ctgrAttrController;
    ...
    @Autowired
    CatalogAttrController catalogAttrController;

    @Autowired
    CatalogAttrServiceImpl catalogAttrService;

    @Test
    public void getStdCtlgAttr() throws Exception {
        long dispCtgrNo = 1008537;
        long prdNo = 0;
        long ctlgNo = 0;
        HttpServletRequest request = setMockHttpServletRequest();
        HttpServletResponse response = setMockHttpServletResponse();
        List<CatalogAttrVO> retList = catalogAttrController.getCtgrStdAttrInfo
                                        (dispCtgrNo, prdNo, ctlgNo);
        //for(List<CatalogAttrVO> )

    }

    @Test
    public void testGetCtgrAttrInfo(){
        long dispCtgrNo = 1008537;
        long prdNo = 0;
        long ctlgNo = 0;
        List<CatalogAttrVO> retList  = catalogAttrService.getCtgrAttrInfo(dispCtgrNo, prdNo, ctlgNo);

    }

    @Test
    public void checkGetPrdBrandInfo() throws Exception{
        CtgrAttrVO brandInfo = ctgrAttrService.getPrdBrandInfo(1245265014);

            System.out.println(brandInfo.getPrdAttrValueCompValueNm());
            System.out.println(brandInfo.getPrdAttrValueCompValueCd());


    }

    @Test
    public void checkGetCtgrAttributeInfo() throws Exception {
        HttpServletRequest request = setMockHttpServletRequest();
        HttpServletResponse response = setMockHttpServletResponse();
        long dispCtgrNo = 1010479;
        long infoTypeCtgrNo = 891011;
        String modifyCtgrYn ="N";
        String tourAttrDisableYn = "N";
        long prdNo = 1245264804;

        List<CtgrAttrVO> ctgrAttrVOList = ctgrAttrController.getCtgrAttributeInfo( dispCtgrNo,infoTypeCtgrNo, modifyCtgrYn,  tourAttrDisableYn,prdNo);

        for(CtgrAttrVO ctgrAttrVO : ctgrAttrVOList){
            System.out.println("getPrdAttrValueNm : "+ctgrAttrVO.getPrdAttrValueNm());
            System.out.println("getPrdAttrValueCd : "+ctgrAttrVO.getPrdAttrValueCd());
            System.out.println("getPrdAttrValueCompValueCd : "+ctgrAttrVO.getPrdAttrValueCompValueCd());
            System.out.println("getPrdAttrValueCompValueNm : "+ctgrAttrVO.getPrdAttrValueCompValueNm());

        }
    }

    @Test
    public void checkGetCodeByAttributeAndValueNm2(){
        HashMap param = new HashMap();
        param.put("infoTypeCtgrNo",0);
        param.put("prdAttrValueNm","상품상세설명 참조");
        param.put("dispCtgrNo",1007853);
        //param.put("infoTypeCtgrNo",1007853);
        ctgrAttrMapper.getCodeByAttributeAndValueNm2(param);
    }

    @Test
    public void checkInsertAttributeValueComp(){
        PdPrdAttrComp pdPrdAttrComp = new PdPrdAttrComp();
        pdPrdAttrComp.setObjPrdNo(99999999);
        pdPrdAttrComp.setPrdAttrCd("23759095");
        pdPrdAttrComp.setPrdAttrValueCd("5963877");
        pdPrdAttrComp.setObjClfCd("01");
        pdPrdAttrComp.setPrdAttrValueNm("상품상세설명 참조");
        pdPrdAttrComp.setPrdImgNo(0);
        pdPrdAttrComp.setCreateNo(0);
        pdPrdAttrComp.setUpdateNo(0);
